...
class GeneratePDFInvoiceCustomer{
    ...
    public function __construct(SalesOrder $salesOrder){
        ...
        $this->directory = 'pdf/invoice/customer/';
    ...
    public function generate($changeDate = false, $invoiceDate = null){
        ...
        if($salesOrder->discount_sales > $salesOrder->discount_company)
            $salesOrder->total_omset = $salesOrder->items->sum('total_price') * (100-$salesOrder->discount_sales)/100;
        else
            $salesOrder->total_omset = $salesOrder->items->sum('total_price') * (100-$salesOrder->discount_company)/100;
        $salesOrder->save();
        $customPaper = array(0,0,684,792);
        $pdf = Pdf::loadView('pdfs.invoiceCustomer', compact('salesOrder','logo'))
